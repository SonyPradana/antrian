...
export async function sendHook(
  ...
  sortedPoliKeys.forEach((poliKey) => {
    const totalQueue =
      groupedCall[poliKey].length + groupedQueue[poliKey].length;
    ...
    if (totalQueue && Number(y2) != totalQueue) {
      console.log(
        `  [OK] ${getDate()}: send poli ${poliKey} with total queue is ${totalQueue} and new queue is ${y2}`,
      );
      fetch(
        `${config.api.queue_url}?poli=${poliKey.toUpperCase()}&antrian=${totalQueue}`,
      )
        .then()
        .catch(console.error);
    ...
    console.log(
      `[INFO] ${getDate()}: data not sent, poli ${poliKey} have ${totalQueue}`,
    );
  });

  sortedPoliKeys.forEach((poliKey) => {
    const lastCalledKode = groupedCall[poliKey].at(-1)?.Kode;
    const lastCalledNumber = lastCalledKode
      ? parseInt(lastCalledKode.replace(/^[a-zA-Z]+/, ''), 10)
      : undefined;
    ...
    if (
      lastCalledKode !== undefined &&
      prevCalledKode !== undefined &&
      prevCalledKode === lastCalledKode
    ) {
      console.log(
        `[INFO] ${getDate()}: data not sent, ${lastCalledKode} not changed. ${prevCalledKode}`,
      );
      return;
    }
    ...
    if (lastCalledNumber !== undefined) {
      console.log(
        `  [OK] ${getDate()}: send poli ${poliKey} with last call is ${lastCalledNumber}`,
      );

      fetch(
        `${config.api.called_url}?poli=${poliKey.toUpperCase()}&antrian=${lastCalledNumber}`,
      )
        .then()
        .catch(console.error);
    ...
    console.log(
      `[INFO] ${getDate()}: data not sent, poli ${poliKey} has empty call`,
    );
  });
...
async function getLastAntrian() {
  ...
  if (false === antrianResponse.ok) {
    throw new Error(
      `Failed to fetch antrian: ${antrianResponse.status} ${antrianResponse.statusText}`,
    );
  }

  const getAntrian = (await antrianResponse.json()) as { data: any[] };
  const antrian = mapToFingerUnits(getAntrian.data);
  const antrian_call = antrian.filter(
    (row) => row.Status.toLowerCase() === 'sudah',
  );
  const antrian_queue = antrian.filter(
    (row) =>
      row.Status.toLowerCase() === 'belum' &&
      false === antrian_call.includes(row),
  );
