...
export function groupByKodePrefix(rows: FingerUnit[]): {
  a: FingerUnit[];
  b: FingerUnit[];
  c: FingerUnit[];
  d: FingerUnit[];
...
export async function sendHook() {
  try {
    ...
    const antrian_call = anttrian.filter(
      (row) => row.Status.toLowerCase() === 'sudah',
    );
    const antrian_queue = anttrian.filter(
      (row) => row.Status.toLowerCase() === 'belum',
    );

    send(groupByKodePrefix(antrian_call), groupByKodePrefix(antrian_queue));
